...
class LatentResizer(nn.Module):
    def __init__(self, in_channels=48, in_blocks=8, out_blocks=8,
                 channels=256, dropout=0.1, attn=True):
        super().__init__()
        ...
        for i in range(in_blocks):
            ...
            self.in_blocks.append(ResBlockEmb(channels, 64, dropout))

        self.out_blocks = nn.ModuleList()
        for i in range(out_blocks):
            if attn and (i == 1 or i == out_blocks - 1):
                self.out_blocks.append(AttnBlock(channels))
            self.out_blocks.append(ResBlockEmb(channels, 64, dropout))
...
def _detect_arch(sd):
    cfg = {"in_channels": 48, "in_blocks": 8, "out_blocks": 8,
           "channels": 256, "dropout": 0.1, "attn": True}
    ...
    # 新缩小架构 (all_blocks)
    if any('resizer.all_blocks' in k for k in sd):
        ids = set()
        for k in sd:
            m = re.match(r'resizer\.all_blocks\.(\d+)\.in_layers\.', k)
            if m: ids.add(int(m.group(1)))
        cfg["in_blocks"] = len(ids)
        cfg["out_blocks"] = 0
        cfg["arch"] = "downscaler"
    else:
    ...
    cfg["attn"] = any('.q.' in k and 'in_blocks' in k for k in sd)
    return cfg
...
# ComfyUI 节点
# ==========================================
class Wan22LatentResizerNode:
    """WAN 2.2 Latent 缩放节点。scale>1 放大，scale<1 缩小，scale=1 不处理。支持 FP8/FP16/合并模型。"""
...
# ==========================================
NODE_CLASS_MAPPINGS = {
    "Wan22LatentResizerNode": Wan22LatentResizerNode,
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "Wan22LatentResizerNode": "WAN 2.2 Latent Resizer",
}
